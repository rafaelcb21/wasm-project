...
OUT_WAT_PATH = "model_int8_esp32.wat"
...
# CONSTANTES NOVAS
# ============================================================
OP_RGB565_TO_RGB888 = 8          # nova op type
FORMAT_FLAG_ADDR   = 0           # endereço na memória linear onde o host escreve o formato
FORMAT_RGB565      = 65          # valor que indica RGB565 (qualquer outro = RGB888)

INPUT_FORMAT_SLOT  = 0           # SLOT onde o host deposita a imagem
RGB888_SLOT        = 1           # SLOT onde a conversão escreve RGB888
...
# ============================================================
# (E-pre) INJETAR CAMADA SINTÉTICA RGB565→RGB888
# ============================================================
graph_input_tid    = int(subgraph.Inputs(0))
graph_input_tensor = subgraph.Tensors(graph_input_tid)
gi_shape  = tensor_shape_list(graph_input_tensor)
rgb_in_h  = gi_shape[1] if len(gi_shape) >= 3 else 128
rgb_in_w  = gi_shape[2] if len(gi_shape) >= 3 else 128
rgb_cin   = gi_shape[3] if len(gi_shape) >= 4 else 3
...
rgb_layer = {
    ...
    "out_h":     int(rgb_in_h),
    "out_w":     int(rgb_in_w),
    "w_off":     0, "has_bias": False, "b_off": 0,
    "has_mulq6": False,
    "mul_off":   0, "shift_off": 0, "q6_off": 0,
    "zx": 0, "zw": 0, "zy": 0,
    "depth_mult": 1,
    "input_slots": [INPUT_FORMAT_SLOT],
    "input_ptrs":  [],
}
layer_params.insert(0, rgb_layer)

# Redireciona tensor de input do subgrafo para SLOT1
for tidx in graph_inputs:
    tensor_to_slot[tidx] = RGB888_SLOT

# Recalcula num_layers, params_bytes e slot_bases
...
# ============================================================
# (E-pre-2) SINCRONIZAR SLOTS APÓS INJEÇÃO DO L0
# ============================================================
# A injeção do L0 deslocou o graph_input de SLOT0 para SLOT1.
# Isso faz todos os slots rotacionarem +1 mod 3:
#   QUANTIZE era [0->0], vira [1->1]
#   CONV L2  era [0->1], vira [1->2]
#   DW   L3  era [1->2], vira [2->0]
#   etc.
#
# Reconstruímos tensor_to_slot aplicando esse shift nos output_slot
# do slot_allocation original, e depois sincronizamos layer_params.

SLOT_SHIFT = RGB888_SLOT  # = 1

# Reconstruir tensor_to_slot do zero com shift aplicado
tensor_to_slot = {}
...
# tensores de saída de cada op: output_slot original + shift
for alloc in slot_allocation:
    label  = alloc['layer']
    op_idx = label_to_op_idx.get(label)
    if op_idx is None:
        continue
    op           = subgraph.Operators(op_idx)
    new_out_slot = (alloc['output_slot'] + SLOT_SHIFT) % NUM_SLOTS
    for j in range(op.OutputsLength()):
        tid = int(op.Outputs(j))
        if tid >= 0:
            tensor_to_slot[tid] = new_out_slot
            print(f"  [REBUILD] tensor {tid} ({alloc['type']} {label}): "
                  f"orig_out={alloc['output_slot']} → new={new_out_slot}")

print(f"  [REBUILD] total: {len(tensor_to_slot)} tensores mapeados")

# Sincronizar layer_params com tensor_to_slot reconstruído
for p in layer_params:
    op_idx = p.get("op_index", -1)
    if op_idx < 0:
        continue

    op      = subgraph.Operators(op_idx)
    in_ids  = [int(k) for k in op.InputsAsNumpy()  if int(k) >= 0
               and not is_constant_tensor(model, subgraph, int(k))]
    out_ids = [int(k) for k in op.OutputsAsNumpy() if int(k) >= 0]

    if in_ids and in_ids[0] in tensor_to_slot:
        p["in_slot"] = tensor_to_slot[in_ids[0]]

    if out_ids and out_ids[0] in tensor_to_slot:
        p["out_slot"] = tensor_to_slot[out_ids[0]]

    # QUANTIZE é in-place: out_slot = in_slot e atualiza pad_t
    if p["optype"] == "QUANTIZE":
        p["out_slot"]    = p["in_slot"]
        p["pad_t"]       = slot_bases[p["in_slot"]]
        p["input_ptrs"]  = [p["pad_t"]]

    # MEAN / SOFTMAX: atualiza pad_t
    elif p["optype"] in ("MEAN", "SOFTMAX"):
        p["pad_t"]      = slot_bases[p["in_slot"]]
        p["input_ptrs"] = [p["pad_t"]]

    # ADD: recalcula ambos input_slots e ptrs
    elif p["optype"] == "ADD" and len(in_ids) >= 2:
        slot_a = tensor_to_slot.get(in_ids[0], p["input_slots"][0])
        slot_b = tensor_to_slot.get(in_ids[1], p["input_slots"][1])
        p["in_slot"]      = slot_a
        p["input_slots"]  = [slot_a, slot_b]
        p["pad_t"]        = slot_bases[slot_a]
        p["pad_b"]        = slot_bases[slot_b]
        p["input_ptrs"]   = [p["pad_t"], p["pad_b"]]

    print(f"  [SYNC] {p['optype']} op_index={op_idx}: "
          f"in_slot={p['in_slot']} out_slot={p['out_slot']}")

# Preenche input_ptrs do layer sintético agora que slot_bases está correto
rgb_layer["input_ptrs"] = [slot_bases[INPUT_FORMAT_SLOT]]

print(f"\n[INFO] Layer sintético RGB565→RGB888 injetado como L0")
print(f"  slot_bases recalculados: {slot_bases}")
print(f"  num_layers agora: {num_layers}")
...
def op_type_name(x):
    return {
        OP_CONV:"CONV", OP_DW:"DW", OP_FC:"FC",
        OP_ADD:"ADD", OP_MEAN:"MEAN", OP_SOFTMAX:"SOFTMAX",
        OP_QUANTIZE:"QUANTIZE", OP_RGB565_TO_RGB888:"RGB565_TO_RGB888"
    }.get(x, str(x))
...
sections.append(";; RGB565_TO_RGB888 (op_type=8):")
sections.append(";;   flags       = endereço na memória onde o host escreve o formato")
sections.append(";;   kh          = valor sentinela (65 = RGB565)")
sections.append(";;   in_ptr      = SLOT0 (onde o host depositou a imagem)")
sections.append(";;   out_ptr     = SLOT1 (onde a RGB888 é escrita)")
sections.append(";;   Lógica WASM: se mem[0] == 65 → converte RGB565→RGB888 e escreve SLOT1")
sections.append(";;               senão → copia SLOT0→SLOT1 como RGB888 direto")
sections.append(";; ")
